...
各連結成分を根付き木として表し、各頂点は親をたどることで代表元へ到達する。同じ代表元を持つ頂点は同じ集合に属する。

集合を統合するときは、一方の根をもう一方の根の子にするだけでよい。ただし無条件に接続すると細長い木になり、代表元探索が遅くなる。そこで次の2つを組み合わせる。

- **union by size**: 小さい木を大きい木の下へ接続し、高さの増加を抑える
- **経路圧縮**: 代表元を探した頂点を直接根へつなぎ替え、次回以降を短縮する

`parentOrSize[root]` に負の集合サイズを保存し、非根には親番号を保存することで、1つの配列で親とサイズを管理する。

### 計算量

経路圧縮と union by size により、償却計算量は1操作あたり $O(\alpha(N))$。`α` は逆アッカーマン関数で、現実的な入力ではほぼ定数とみなせる。
...
Union-Find は辺を削除する処理には通常対応できない。
...
問題の難易度が上がった場合、次のアルゴリズムを別途追加するとよい。
